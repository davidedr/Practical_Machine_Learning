...
Now let's rebuild the data frame taking the original dataframe and converting all values to `numeric` (apart from the target variable, stored in th 153-rd columns of the dataframe):
```{r}
...
## In-sample
Here's the in-sample confusion matrix of the fitted model. Looks to work pretty well on the training set, keeping the in-sample classification error lower than 1.8%:
```{r}
model.rf$finalModel
```

Another assessment of the accuracy of the predictor vs the numer of variables can be computes by *bootstrap*:
```{r}
plot(model.rf, main = 'Accuracy of the Model')
```

## Cross- validation
Let's check the model on th cross validation set:
```{r}
predict_cv <- predict(model.rf, newdata = data_cross)
table(predict_cv, data_cross$classe)
```

We have an error of **`r mean(predict_cv == data_cross$classe)' %**.

## Out-of-sample
To work on the test set, we need to perform on it oall the proccessing and clean-up we did on the training data set: 
```{r}
rm(data_test_file)
